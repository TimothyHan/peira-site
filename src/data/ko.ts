...
export const referenceKo: readonly RefGroup[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { term: "multipart", note: "{fields?, files?: [{field, path, mimetype?, filename?}]} — JSON 본문 대신 multipart/form-data를 보냅니다(둘 다는 불가). files[].path는 케이스 디렉터리 기준 상대 경로입니다: 저장소 안의 보통 파일이며 바이트를 인라인으로 담지 않습니다; 파일이 없거나 256 KB를 넘으면 validate가 거부합니다. mimetype을 명시하므로 잘못된 형식 거부도 케이스가 됩니다. 증거 로그에는 이름과 크기만 남고 내용은 결코 남지 않습니다." },
