...
export const cliCommandsKo: readonly CliCommand[] = [
  ...
  { name: "stats", synopsis: "peira stats [casesDir] [--openapi <spec.json>]", description: "DSL 커버리지, 반복되는 이스케이프 해치 형태, 그리고 거부 균형 — 인텐트마다 케이스 수, 긍정(기대 상태 < 400), 부정(≥ 400), 부정 오라클($absent / $notContains)을 표로 내고, 해피 패스만 테스트하는 인텐트를 한 줄로 지목합니다. 스위트는 해피 패스 하나씩 긍정 쪽으로 기울어 갑니다; 이것이 매 실행마다의 가드입니다. --openapi를 주면 엔드포인트 커버리지 — 케이스가 없는 엔드포인트 — 를 냅니다." },
...
export const referenceKo: readonly RefGroup[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { term: "headers", note: "케이스 자체의 요청 헤더 — {\"Accept\": \"text/html\", \"Accept-Language\": \"ko\"} — 보간됩니다. 가장 낮은 우선순위로 적용됩니다: 주체의 인증 헤더가 이를 덮어쓰고(케이스는 자신의 Authorization을 바꿀 수 없습니다) content-type은 본문 종류가 소유합니다. content-length, transfer-encoding, host는 거부되며, HTTP 런타임이 소리 없이 고쳐 쓰는 Sec-* / Proxy-*도 거부됩니다." },
      { term: "form", note: "{name: value}을 application/x-www-form-urlencoded로 보냅니다 — 하이드레이션 전 브라우저가 보내는 네이티브 폼 전송입니다. body·multipart와 배타적이며, password 필드는 증거에서 가려집니다." },
      { term: "multipart", note: "{fields?, files?: [{field, path | bytes, mimetype?, filename?}]} — JSON 본문 대신 multipart/form-data를 보냅니다(둘 다는 불가). files[].path는 케이스 디렉터리 기준 상대 경로입니다: 저장소 안의 보통 파일이며 바이트를 인라인으로 담지 않습니다; 파일이 없거나 256 KB를 넘으면 validate가 거부합니다. mimetype을 명시하므로 잘못된 형식 거부도 케이스가 됩니다. 증거 로그에는 이름과 크기만 남고 내용은 결코 남지 않습니다." },
  ...
  },
  {
    ...
    entries: [
      ...
      { term: "oracle", note: "{statusOnly: \"<이유>\"} — 단정이 아니라 메모입니다: 이 케이스는 일부러 상태 코드만 단정합니다(추론할 수 없도록 동일한 404들). 약한 오라클 린트가 이를 받아들이고, render가 출력하며, body나 headers 단정 옆에서는 거부됩니다." },
      ...
      { term: "{\"$contains\": …}", note: "매처: 부분 문자열을 포함하는 문자열 — 또는 목록의 모든 부분 문자열을 포함(all of; 빠진 것마다 별도 diff). content-type 매처이자, 텍스트 본문의 오라클입니다: HTML 등 JSON이 아닌 응답은 문자열로 도착합니다. 함정 하나: 서버 렌더링 React는 인접한 텍스트 표현식 사이에 <!-- -->를 끼우므로, 보간을 가로지르는 보이는 텍스트가 아니라 한 표현식에서 나온 텍스트나 안정적인 속성값을 단정하세요." },
      ...
      { term: "{\"$text\": {contains, notContains}}", note: "매처: 본문을 텍스트로 — 태그와 주석을 벗기고 공백을 접은 뒤 — 모두 포함하고 하나도 포함하지 않아야 합니다. 본문 전체에만 씁니다. 서버 렌더링 React는 텍스트 사이에 <!-- -->를 끼우므로 원문에 대한 $contains는 \"총 2건\"을 놓칩니다; $text는 찾아내며, 같은 텍스트 본문에 contains와 notContains를 함께 쓸 수 있는 유일한 자리입니다." },
